...
def test_convert_full_and_fast_skips_missing_portlist_without_tcp_ports(tmp_path, monkeypatch):
    data_objects = tmp_path / "data-objects"
    vt_path = tmp_path / "vulnerability-tests"
    scan_configs = data_objects / "scan-configs"
    scan_configs.mkdir(parents=True)
...
def test_convert_full_and_fast_retries_with_legacy_scannerctl_cli(tmp_path, monkeypatch):
    data_objects = tmp_path / "data-objects"
    vt_path = tmp_path / "vulnerability-tests"
    scan_configs = data_objects / "scan-configs"
    scan_configs.mkdir(parents=True)
